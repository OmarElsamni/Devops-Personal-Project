...
if [ -z "$AWS_ACCOUNT_ID" ]; then
    print_error "Failed to get AWS account ID. Is AWS CLI configured?"
    exit 1
fi
...
# Build backend for linux/amd64 platform
print_info "Building backend image for linux/amd64..."
cd backend
docker build --platform linux/amd64 -t $BACKEND_IMAGE:$IMAGE_TAG .
docker tag $BACKEND_IMAGE:$IMAGE_TAG $BACKEND_IMAGE:latest
cd ..

# Build frontend for linux/amd64 platform
